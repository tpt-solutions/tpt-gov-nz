//! Cross-department consent verification for the Ministry for the Environment department node.
//!
//! Mirrors `policies/mfe.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::MfeError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(MfeError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MfeError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn mfe_grant_allows_mfe_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["mfe:emissions".into()]);
        let scopes = vec!["mfe:emissions".into()];
