...
use crate::models::{ProgrammesEntity,InsightsEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, women_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET women_id = EXCLUDED.women_id
...
pub async fn upsert_programmes(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO women_programmes (citizen_id, programme_name, status, year)
           VALUES ($1, $2, $3, $4)
...
pub async fn upsert_insights(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO women_insights (citizen_id, topic, summary, published)
           VALUES ($1, $2, $3, $4)
