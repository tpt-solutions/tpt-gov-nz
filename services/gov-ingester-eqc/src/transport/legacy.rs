...
/// Connection details for the real Earthquake Commission (Toka Tū Ake) legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawEqcBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Earthquake Commission (Toka Tū Ake) legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
