...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.eqc_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for claims in &t.claims {
            count(
                db::upsert_claims(pool, citizen_id, claims).await?,
                &mut inserted,
                &mut updated,
            );
        }
        ...
        if let Some(cover) = &t.cover {
            count(
                db::upsert_cover(pool, citizen_id, cover).await?,
                &mut inserted,
                &mut updated,
            );
        }
