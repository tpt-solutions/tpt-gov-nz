...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "mot" }))
}
...
) -> Result<Json<ResolveResponse>, MotError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(MotError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.mot_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(MotError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let strategies = if has_scope("mot:strategies") {
        Some(db::fetch_strategies(&pool, citizen.id).await?)
    } else {
        None
    };

    let strategies_json = strategies.as_ref().map(|rows| {
    ...
    let programmes = if has_scope("mot:programmes") {
