//! Integration + unit tests for the Ministry of Transport department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://mot:mot_dev_password@localhost:5432/mot cargo test -p gov-dept-mot
//! ```
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.mot_id, "MOT-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["mot_id"], "MOT-100001");
    assert!(data["strategies"].is_null());
    assert!(data["programmes"].is_null());
}

#[sqlx::test]
async fn fetch_data_strategies_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["mot:strategies"]).await;
    let arr = data["strategies"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["title"], "Te Tangi a Te Manu");
}

#[sqlx::test]
async fn fetch_data_programmes_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["mot:programmes"]).await;
    let arr = data["programmes"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["name"], "Road maintenance boost");
}
...
async fn action_requestProgrammeInfo_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, MotError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["mot:strategies","mot:programmes"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["mot_id"], "MOT-100001");
    assert_eq!(body["strategies"].as_array().unwrap().len(), 1);
