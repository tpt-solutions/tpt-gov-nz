...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "women" }))
}
...
) -> Result<Json<ResolveResponse>, WomenError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(WomenError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.women_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(WomenError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let programmes = if has_scope("women:programmes") {
    ...
    let insights = if has_scope("women:insights") {
        Some(db::fetch_insights(&pool, citizen.id).await?)
    } else {
        None
    };

    let insights_json = insights.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "topic": c.c,
            "summary": c.c,
