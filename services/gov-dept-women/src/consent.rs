//! Cross-department consent verification for the Ministry for Women department node.
//!
//! Mirrors `policies/women.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::WomenError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(WomenError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(WomenError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn women_grant_allows_women_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["women:programmes".into()]);
        let scopes = vec!["women:programmes".into()];
