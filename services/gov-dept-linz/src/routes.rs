...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "linz" }))
}
...
) -> Result<Json<ResolveResponse>, LinzError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(LinzError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.customer_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(LinzError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let titles = if has_scope("linz:titles") {
        Some(db::fetch_titles(&pool, citizen.id).await?)
    } else {
        None
    };

    let ownership = if has_scope("linz:ownership") {
        Some(db::fetch_ownership(&pool, citizen.id).await?)
    } else {
        None
    };

    let titles_json = titles.as_ref().map(|rows| {
        json!(rows.iter().map(|t| json!({
            "titleNumber": t.title_number,
            "propertyAddress": t.property_address,
            "landAreaSqm": t.land_area_sqm,
            "estateType": t.estate_type,
        })).collect::<Vec<_>>())
    });

    let ownership_json = ownership.as_ref().map(|rows| {
        json!(rows.iter().map(|o| json!({
            "titleNumber": o.title_number,
            "ownershipShare": o.ownership_share,
            "registeredOwners": o.registered_owners,
        })).collect::<Vec<_>>())
    });

    Ok(Json(json!({
        "customerId": citizen.customer_id,
        "titles": titles_json,
