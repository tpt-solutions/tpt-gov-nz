...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "defence" }))
}
...
) -> Result<Json<ResolveResponse>, DefenceError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(DefenceError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.defence_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(DefenceError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let procurements = if has_scope("defence:procurements") {
        Some(db::fetch_procurements(&pool, citizen.id).await?)
    } else {
        None
    };

    let procurements_json = procurements.as_ref().map(|rows| {
    ...
    let bases = if has_scope("defence:bases") {
        Some(db::fetch_bases(&pool, citizen.id).await?)
    } else {
        None
    };

    let bases_json = bases.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "name": c.c,
            "location": c.c,
            "status": c.c,
    ...
    Ok(Json(json!({
        "defence_id": citizen.defence_id,
        "procurements": procurements_json,
        "bases": bases_json,
    })))
}
