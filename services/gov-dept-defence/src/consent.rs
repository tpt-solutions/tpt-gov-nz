//! Cross-department consent verification for the Ministry of Defence department node.
//!
//! Mirrors `policies/defence.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::DefenceError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "defence"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(DefenceError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(DefenceError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn defence_grant_allows_defence_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["defence:procurements".into()]);
        let scopes = vec!["defence:procurements".into()];
