...
/// Connection details for the real MSD legacy system integration.
///
/// In production the ingester connects to the MSD legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMsdBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "MSD legacy transport not yet wired up (target: {}). \
