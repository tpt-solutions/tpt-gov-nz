...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        if let Some(studylink) = &t.studylink {
            count(
                db::upsert_studylink(pool, citizen_id, studylink).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for event in &t.case_events {
            count(
                db::upsert_case_event(pool, citizen_id, event).await?,
                &mut inserted,
                &mut updated,
            );
        }
