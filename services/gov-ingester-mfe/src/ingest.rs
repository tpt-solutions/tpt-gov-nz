...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.mfe_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for emissions in &t.emissions {
            count(
                db::upsert_emissions(pool, citizen_id, emissions).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for reports in &t.reports {
            count(
                db::upsert_reports(pool, citizen_id, reports).await?,
                &mut inserted,
                &mut updated,
            );
        }
