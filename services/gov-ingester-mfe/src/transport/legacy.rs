...
/// Connection details for the real Ministry for the Environment legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMfeBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Ministry for the Environment legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
