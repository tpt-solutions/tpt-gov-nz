//! Cross-department consent verification for the MSD department node.
//!
//! Mirrors `policies/msd.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::MsdError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(MsdError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MsdError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn msd_grant_allows_msd_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["msd:studylink".into()]);
        let scopes = vec!["msd:studylink".into()];
    ...
    fn wrong_dept_grant_denies() {
        ...
        let scopes = vec!["msd:case-history".into()];
        let input = ConsentInput {
            requesting_dept_id: "ird",
            citizen_did: "did:gov:nz:test-citizen-001",
            requested_scopes: &scopes,
