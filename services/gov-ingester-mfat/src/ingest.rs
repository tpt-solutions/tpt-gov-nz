...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.mfat_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for overseas_missions in &t.overseas_missions {
            count(
                db::upsert_overseas_missions(pool, citizen_id, overseas_missions).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for travel_advisories in &t.travel_advisories {
            count(
                db::upsert_travel_advisories(pool, citizen_id, travel_advisories).await?,
                &mut inserted,
                &mut updated,
            );
        }
