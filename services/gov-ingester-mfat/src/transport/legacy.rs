...
/// Connection details for the real Ministry of Foreign Affairs and Trade legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMfatBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Ministry of Foreign Affairs and Trade legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
