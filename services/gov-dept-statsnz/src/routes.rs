...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "statsnz" }))
}
...
) -> Result<Json<ResolveResponse>, StatsnzError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(StatsnzError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.stats_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(StatsnzError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let census = if has_scope("statsnz:census") {
        Some(db::fetch_census(&pool, citizen.id).await?)
    } else {
        None
    };

    let profile = if has_scope("statsnz:profile") {
        Some(db::fetch_profile(&pool, citizen.id).await?)
    } else {
        None
    };

    let census_json = census.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "censusYear": c.census_year,
            "dwellingType": c.dwelling_type,
    ...
    let profile_json = profile.as_ref().map(|p| {
        json!({
            "dataSummary": p.data_summary,
            "recordCount": p.record_count,
            "lastUpdated": p.last_updated.to_string(),
        })
    });

    Ok(Json(json!({
        "statsId": citizen.stats_id,
        "census": census_json,
