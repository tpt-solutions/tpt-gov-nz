//! Cross-department consent verification for the Statistics New Zealand department node.
//!
//! Mirrors `policies/statsnz.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::StatsnzError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "statsnz"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(StatsnzError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(StatsnzError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn statsnz_grant_allows_statsnz_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["statsnz:census".into()]);
        let scopes = vec!["statsnz:census".into()];
