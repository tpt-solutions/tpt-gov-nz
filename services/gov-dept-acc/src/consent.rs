//! Cross-department consent verification for the ACC department node.
//!
//! Mirrors `policies/acc.rego` so the same allow/deny decision can be made locally
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(AccError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(AccError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn acc_grant_allows_acc_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["acc:claims".into()]);
        let scopes = vec!["acc:claims".into()];
