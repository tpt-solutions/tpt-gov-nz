...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.oranga_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for care_placements in &t.care_placements {
            count(
                db::upsert_care_placements(pool, citizen_id, care_placements).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for support_services in &t.support_services {
            count(
                db::upsert_support_services(pool, citizen_id, support_services).await?,
                &mut inserted,
                &mut updated,
            );
        }
