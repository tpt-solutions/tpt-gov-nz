...
/// Connection details for the real ACC legacy system integration.
///
/// In production the ingester connects to the ACC legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawAccBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "ACC legacy transport not yet wired up (target: {}). \
