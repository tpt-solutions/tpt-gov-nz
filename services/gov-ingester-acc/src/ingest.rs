...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        for claim in &t.claims {
            count(db::upsert_claim(pool, citizen_id, claim).await?, &mut inserted, &mut updated);
        }
        if let Some(ent) = &t.entitlements {
            count(
                db::upsert_entitlement(pool, citizen_id, ent).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for rehab in &t.rehabilitation {
            count(
                db::upsert_rehabilitation(pool, citizen_id, rehab).await?,
                &mut inserted,
                &mut updated,
            );
        }
