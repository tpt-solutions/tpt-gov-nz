...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.pacific_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for programmes in &t.programmes {
            count(
                db::upsert_programmes(pool, citizen_id, programmes).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for language_services in &t.language_services {
            count(
                db::upsert_language_services(pool, citizen_id, language_services).await?,
                &mut inserted,
                &mut updated,
            );
        }
