...
//!
//! When the WINZ node is deployed with an OPA sidecar (`OPA_URL` set) and built with
//! the `opa` feature, the route layer defers the consent decision to the deployed
//! `policies/winz.rego`. The local [`crate::consent`] evaluation is always available
...
/// Query the OPA sidecar for the `winz.consent` decision.
///
/// Returns `None` when OPA is not configured or unavailable (caller falls back to
/// the local [`crate::consent`] evaluation). Only compiled with the `opa` feature.
#[cfg(feature = "opa")]
