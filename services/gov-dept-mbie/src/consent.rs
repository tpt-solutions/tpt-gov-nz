//! Cross-department consent verification for the MBIE department node.
//!
//! Mirrors `policies/mbie.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::MbieError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(MbieError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MbieError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn mbie_grant_allows_mbie_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["mbie:business".into()]);
        let scopes = vec!["mbie:business".into()];
