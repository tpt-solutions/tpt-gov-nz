//! Cross-department consent verification for the Police department node.
//!
//! Mirrors `policies/police.rego` so the same allow/deny decision can be made
//! locally (and in unit tests) without a running OPA sidecar.

use crate::error::PoliceError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "police"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(PoliceError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(PoliceError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn police_grant_allows_police_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["police:infringements".into()]);
        let scopes = vec!["police:infringements".into()];
