...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "caa" }))
}
...
) -> Result<Json<ResolveResponse>, CaaError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(CaaError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.caa_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(CaaError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let licences = if has_scope("caa:licences") {
        Some(db::fetch_licences(&pool, citizen.id).await?)
    } else {
        None
    };

    let licences_json = licences.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "licenceNo": c.c,
            "category": c.c,
            "status": c.c,
            "expires": c.c.to_string(),
        })).collect::<Vec<_>>())
    });

    let aircraft = if has_scope("caa:aircraft") {
        Some(db::fetch_aircraft(&pool, citizen.id).await?)
    } else {
        None
    };

    let aircraft_json = aircraft.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "registration": c.c,
            "aircraftType": c.c,
            "status": c.c,
