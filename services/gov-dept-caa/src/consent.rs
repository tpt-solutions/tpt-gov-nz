//! Cross-department consent verification for the Civil Aviation Authority department node.
//!
//! Mirrors `policies/caa.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::CaaError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(CaaError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(CaaError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn caa_grant_allows_caa_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["caa:licences".into()]);
        let scopes = vec!["caa:licences".into()];
