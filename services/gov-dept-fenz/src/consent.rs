//! Cross-department consent verification for the Fire and Emergency New Zealand department node.
//!
//! Mirrors `policies/fenz.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::FenzError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(FenzError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(FenzError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn fenz_grant_allows_fenz_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["fenz:fire-safety".into()]);
        let scopes = vec!["fenz:fire-safety".into()];
