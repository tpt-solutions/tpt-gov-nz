...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "fenz" }))
}
...
) -> Result<Json<ResolveResponse>, FenzError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(FenzError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.fenz_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(FenzError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let fire_safety = if has_scope("fenz:fire-safety") {
        Some(db::fetch_fire_safety(&pool, citizen.id).await?)
    } else {
        None
    };

    let fire_safety_json = fire_safety.as_ref().map(|p| {
        json!({
            "property": c.p,
            "grade": c.p,
            "lastInspection": c.p.to_string(),
        })
    });

    let incidents = if has_scope("fenz:incidents") {
        Some(db::fetch_incidents(&pool, citizen.id).await?)
    } else {
        None
    };

    let incidents_json = incidents.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "reference": c.c,
            "incidentType": c.c,
            "incidentDate": c.c.to_string(),
            "status": c.c,
        })).collect::<Vec<_>>())
    });

    Ok(Json(json!({
        "fenz_id": citizen.fenz_id,
        "fire_safety": fire_safety_json,
