...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.caa_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for licences in &t.licences {
            count(
                db::upsert_licences(pool, citizen_id, licences).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for aircraft in &t.aircraft {
            count(
                db::upsert_aircraft(pool, citizen_id, aircraft).await?,
                &mut inserted,
                &mut updated,
            );
        }
