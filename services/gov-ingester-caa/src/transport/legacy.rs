...
/// Connection details for the real Civil Aviation Authority legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawCaaBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Civil Aviation Authority legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
