...
use crate::models::{LicencesEntity,AircraftEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, caa_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET caa_id = EXCLUDED.caa_id
...
pub async fn upsert_licences(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &LicencesEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO caa_licences (citizen_id, licence_no, category, status, expires)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (citizen_id, licence_no) DO UPDATE SET
              licence_no = EXCLUDED.licence_no, category = EXCLUDED.category, status = EXCLUDED.status, expires = EXCLUDED.expires
...
pub async fn upsert_aircraft(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO caa_aircraft (citizen_id, registration, aircraft_type, status)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (citizen_id, registration) DO UPDATE SET
              registration = EXCLUDED.registration, aircraft_type = EXCLUDED.aircraft_type, status = EXCLUDED.status
           RETURNING (xmax = 0) AS inserted"#,
        citizen_id,
        e.registration,
        e.aircraft_type,
