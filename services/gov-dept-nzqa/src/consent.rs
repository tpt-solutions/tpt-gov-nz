//! Cross-department consent verification for the NZQA department node.
//!
//! Mirrors `policies/nzqa.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::NzqaError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(NzqaError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(NzqaError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn nzqa_grant_allows_nzqa_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["nzqa:qualifications".into()]);
        let scopes = vec!["nzqa:qualifications".into()];
