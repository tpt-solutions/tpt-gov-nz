...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "nzqa" }))
}
...
) -> Result<Json<ResolveResponse>, NzqaError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(NzqaError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.nsn,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(NzqaError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let qualifications = if has_scope("nzqa:qualifications") {
        Some(db::fetch_qualifications(&pool, citizen.id).await?)
    } else {
        None
    };

    let transcript = if has_scope("nzqa:transcripts") {
        db::fetch_transcript(&pool, citizen.id).await?
    } else {
        None
    };

    let qualifications_json = qualifications.as_ref().map(|rows| {
        json!(rows.iter().map(|q| json!({
            "qualificationId": q.qualification_id,
            "title": q.title,
            "level": q.level,
            "awardedDate": q.awarded_date.to_string(),
            "provider": q.provider,
        })).collect::<Vec<_>>())
    });

    let transcript_json = transcript.as_ref().map(|row| {
        json!({
            "recordSummary": row.record_summary,
            "totalCredits": row.total_credits,
            "creditSummary": row.credit_summary,
        })
    });
