...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "oranga" }))
}
...
) -> Result<Json<ResolveResponse>, OrangaError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(OrangaError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.oranga_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(OrangaError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let care_placements = if has_scope("oranga:care-placements") {
        Some(db::fetch_care_placements(&pool, citizen.id).await?)
    } else {
        None
    };

    let care_placements_json = care_placements.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "placementType": c.c,
            "startDate": c.c.to_string(),
            "region": c.c,
        })).collect::<Vec<_>>())
    });

    let support_services = if has_scope("oranga:support-services") {
        Some(db::fetch_support_services(&pool, citizen.id).await?)
    } else {
        None
    };

    let support_services_json = support_services.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "service": c.c,
            "status": c.c,
            "nextReview": c.c.to_string(),
        })).collect::<Vec<_>>())
    });

    Ok(Json(json!({
        "oranga_id": citizen.oranga_id,
        "care_placements": care_placements_json,
        "support_services": support_services_json,
    })))
}
