//! Integration + unit tests for the Oranga Tamariki department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://oranga:oranga_dev_password@localhost:5432/oranga cargo test -p gov-dept-oranga
//! ```
...
use crate::{build_app, OrangaError};
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.oranga_id, "OT-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["oranga_id"], "OT-100001");
    assert!(data["care_placements"].is_null());
    assert!(data["support_services"].is_null());
}

#[sqlx::test]
async fn fetch_data_care_placements_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["oranga:care-placements"]).await;
    let arr = data["care_placements"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["placementType"], "Whānau placement");
}

#[sqlx::test]
async fn fetch_data_support_services_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["oranga:support-services"]).await;
    let arr = data["support_services"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["service"], "Intensive support");
}
...
async fn action_requestSupportReview_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, OrangaError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["oranga:care-placements","oranga:support-services"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["oranga_id"], "OT-100001");
    assert_eq!(body["care_placements"].as_array().unwrap().len(), 1);
    assert_eq!(body["support_services"].as_array().unwrap().len(), 1);
}
