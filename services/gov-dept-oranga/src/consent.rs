//! Cross-department consent verification for the Oranga Tamariki department node.
//!
//! Mirrors `policies/oranga.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::OrangaError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "oranga"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(OrangaError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(OrangaError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn oranga_grant_allows_oranga_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["oranga:care-placements".into()]);
        let scopes = vec!["oranga:care-placements".into()];
