//! Integration + unit tests for the Education Review Office department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://ero:ero_dev_password@localhost:5432/ero cargo test -p gov-dept-ero
//! ```
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.ero_id, "ERO-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["ero_id"], "ERO-100001");
    assert!(data["reviews"].is_null());
    assert!(data["reports"].is_null());
}

#[sqlx::test]
async fn fetch_data_reviews_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["ero:reviews"]).await;
    let arr = data["reviews"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["school"], "Porirua College");
}

#[sqlx::test]
async fn fetch_data_reports_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["ero:reports"]).await;
    let arr = data["reports"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["title"], "Porirua College annual report");
}

// ── actions ───────────────────────────────────────────────────────────────────────

#[sqlx::test]
async fn action_requestReportCopy_valid(pool: PgPool) {
...
#[sqlx::test]
async fn action_requestReportCopy_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, EroError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["ero:reviews","ero:reports"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["ero_id"], "ERO-100001");
