//! Cross-department consent verification for the Education Review Office department node.
//!
//! Mirrors `policies/ero.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::EroError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(EroError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(EroError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn ero_grant_allows_ero_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["ero:reviews".into()]);
        let scopes = vec!["ero:reviews".into()];
