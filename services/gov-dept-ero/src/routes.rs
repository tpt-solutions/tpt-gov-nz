...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "ero" }))
}
...
) -> Result<Json<ResolveResponse>, EroError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(EroError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.ero_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(EroError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let reviews = if has_scope("ero:reviews") {
        Some(db::fetch_reviews(&pool, citizen.id).await?)
    } else {
        None
    };

    let reviews_json = reviews.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "school": c.c,
            "rating": c.c,
            "reviewDate": c.c.to_string(),
            "nextReview": c.c.to_string(),
        })).collect::<Vec<_>>())
    });

    let reports = if has_scope("ero:reports") {
