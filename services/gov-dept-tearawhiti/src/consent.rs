//! Cross-department consent verification for the Te Arawhiti department node.
//!
//! Mirrors `policies/tearawhiti.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::TearawhitiError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "tearawhiti"
...
pub fn verify_access(
...
) -> Result<(), TearawhitiError> {
    if !evaluate_allow(input) {
        return Err(TearawhitiError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(TearawhitiError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn tearawhiti_grant_allows_tearawhiti_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["tearawhiti:treaty-settlements".into()]);
        let scopes = vec!["tearawhiti:treaty-settlements".into()];
