...
use crate::{
    error::IngestError,
    raw::RawTearawhitiBatch,
    transport::IngesterTransport,
};

/// Connection details for the real Te Arawhiti legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawTearawhitiBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Te Arawhiti legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
