...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.tearawhiti_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for treaty_settlements in &t.treaty_settlements {
            count(
                db::upsert_treaty_settlements(pool, citizen_id, treaty_settlements).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for engagements in &t.engagements {
            count(
                db::upsert_engagements(pool, citizen_id, engagements).await?,
                &mut inserted,
                &mut updated,
            );
        }
