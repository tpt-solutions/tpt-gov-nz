...
/// Reads a raw Te Kawa Mataaho Public Service Commission batch from a JSON fixture file on disk (dev/demo/CI).
...
impl IngesterTransport for MockTransport {
    async fn pull(&self) -> Result<RawPublicserviceBatch, IngestError> {
        ...
        let batch = serde_json::from_slice::<RawPublicserviceBatch>(&bytes)
