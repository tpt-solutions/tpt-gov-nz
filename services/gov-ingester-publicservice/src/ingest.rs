...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.psc_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for workforce in &t.workforce {
            count(
                db::upsert_workforce(pool, citizen_id, workforce).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for agency_ratings in &t.agency_ratings {
            count(
                db::upsert_agency_ratings(pool, citizen_id, agency_ratings).await?,
                &mut inserted,
                &mut updated,
            );
        }
