...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.doc_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for permit in &t.permits {
            count(
                db::upsert_permit(pool, citizen_id, permit).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for concession in &t.concessions {
            count(
                db::upsert_concession(pool, citizen_id, concession).await?,
                &mut inserted,
                &mut updated,
            );
        }
