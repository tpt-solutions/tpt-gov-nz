...
/// Connection details for the real DOC legacy system integration.
///
/// In production the ingester connects to the DOC legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawDocBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "DOC legacy transport not yet wired up (target: {}). \
