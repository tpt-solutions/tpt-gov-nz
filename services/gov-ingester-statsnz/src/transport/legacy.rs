...
/// Connection details for the real Statistics New Zealand legacy system integration.
///
/// In production the ingester connects to the Stats NZ legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawStatsnzBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Stats NZ legacy transport not yet wired up (target: {}). \
