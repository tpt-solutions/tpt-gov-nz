...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.stats_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for census in &t.census {
            count(
                db::upsert_census(pool, citizen_id, census).await?,
                &mut inserted,
                &mut updated,
            );
        }
        ...
        if let Some(profile) = &t.profile {
            count(
                db::upsert_profile(pool, citizen_id, profile).await?,
                &mut inserted,
                &mut updated,
            );
        }
