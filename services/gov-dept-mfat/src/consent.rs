//! Cross-department consent verification for the Ministry of Foreign Affairs and Trade department node.
//!
//! Mirrors `policies/mfat.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::MfatError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(MfatError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MfatError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn mfat_grant_allows_mfat_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["mfat:overseas-missions".into()]);
        let scopes = vec!["mfat:overseas-missions".into()];
