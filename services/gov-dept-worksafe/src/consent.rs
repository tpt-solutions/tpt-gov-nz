//! Cross-department consent verification for the WorkSafe New Zealand department node.
//!
//! Mirrors `policies/worksafe.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::WorksafeError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "worksafe"
...
pub fn verify_access(
...
) -> Result<(), WorksafeError> {
    if !evaluate_allow(input) {
        return Err(WorksafeError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(WorksafeError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn worksafe_grant_allows_worksafe_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["worksafe:inspections".into()]);
        let scopes = vec!["worksafe:inspections".into()];
