//! Integration + unit tests for the WorkSafe New Zealand department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://worksafe:worksafe_dev_password@localhost:5432/worksafe cargo test -p gov-dept-worksafe
//! ```
...
use crate::{build_app, WorksafeError};
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.worksafe_id, "WS-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["worksafe_id"], "WS-100001");
    assert!(data["inspections"].is_null());
    assert!(data["investigations"].is_null());
}

#[sqlx::test]
async fn fetch_data_inspections_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["worksafe:inspections"]).await;
    let arr = data["inspections"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["reference"], "WS-I-2026-003");
}

#[sqlx::test]
async fn fetch_data_investigations_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["worksafe:investigations"]).await;
    let arr = data["investigations"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["reference"], "WS-INV-2026-011");
}

// ── actions ───────────────────────────────────────────────────────────────────────

#[sqlx::test]
async fn action_requestInvestigationUpdate_valid(pool: PgPool) {
    let resp = crate::actions::execute(
        &pool,
        citizen_id(&pool).await,
        "request-investigation-update",
...
#[sqlx::test]
async fn action_requestInvestigationUpdate_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, WorksafeError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["worksafe:inspections","worksafe:investigations"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["worksafe_id"], "WS-100001");
