...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "worksafe" }))
}
...
) -> Result<Json<ResolveResponse>, WorksafeError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(WorksafeError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.worksafe_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
...
) -> Result<Json<Value>, WorksafeError> {
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(WorksafeError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let inspections = if has_scope("worksafe:inspections") {
        Some(db::fetch_inspections(&pool, citizen.id).await?)
    } else {
        None
    };

    let inspections_json = inspections.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "reference": c.c,
            "site": c.c,
            "inspectionDate": c.c.to_string(),
            "outcome": c.c,
        })).collect::<Vec<_>>())
    });

    let investigations = if has_scope("worksafe:investigations") {
        Some(db::fetch_investigations(&pool, citizen.id).await?)
    } else {
        None
    };

    let investigations_json = investigations.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "reference": c.c,
            "matter": c.c,
            "status": c.c,
            "openedDate": c.c.to_string(),
        })).collect::<Vec<_>>())
    });

    Ok(Json(json!({
        "worksafe_id": citizen.worksafe_id,
