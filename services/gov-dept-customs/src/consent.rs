//! Cross-department consent verification for the Customs department node.
//!
//! Mirrors `policies/customs.rego` so the same allow/deny decision can be made locally
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "customs"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(CustomsError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(CustomsError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn customs_grant_allows_customs_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["customs:travel".into()]);
        let scopes = vec!["customs:travel".into()];
