...
use crate::{
    error::IngestError,
    raw::RawRegulationBatch,
    transport::IngesterTransport,
};

/// Connection details for the real Ministry for Regulation legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawRegulationBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Ministry for Regulation legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
