//! Integration + unit tests for the The Treasury department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://treasury:treasury_dev_password@localhost:5432/treasury cargo test -p gov-dept-treasury
//! ```
...
use crate::{build_app, TreasuryError};
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.treasury_id, "TRE-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["treasury_id"], "TRE-100001");
    assert!(data["budget"].is_null());
    assert!(data["economic_outlook"].is_null());
}

#[sqlx::test]
async fn fetch_data_budget_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["treasury:budget"]).await;
    let arr = data["budget"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["fiscalYear"], 2026);
}

#[sqlx::test]
async fn fetch_data_economic_outlook_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["treasury:economic-outlook"]).await;
    let obj = data["economic_outlook"].as_object().unwrap();
    assert_eq!(obj["forecastYear"], 2026);
}

// ── actions ───────────────────────────────────────────────────────────────────────

#[sqlx::test]
async fn action_requestEconomicBrief_valid(pool: PgPool) {
    let resp = crate::actions::execute(
        &pool,
        citizen_id(&pool).await,
        "request-economic-brief",
...
#[sqlx::test]
async fn action_requestEconomicBrief_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, TreasuryError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["treasury:budget","treasury:economic-outlook"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["treasury_id"], "TRE-100001");
    assert_eq!(body["budget"].as_array().unwrap().len(), 1);
    assert!(body["economic_outlook"].as_object().is_some());
}
