...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "treasury" }))
}
...
) -> Result<Json<ResolveResponse>, TreasuryError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(TreasuryError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.treasury_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(TreasuryError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let budget = if has_scope("treasury:budget") {
        Some(db::fetch_budget(&pool, citizen.id).await?)
    } else {
        None
    };

    let budget_json = budget.as_ref().map(|rows| {
    ...
    let economic_outlook = if has_scope("treasury:economic-outlook") {
        Some(db::fetch_economic_outlook(&pool, citizen.id).await?)
    } else {
        None
    };

    let economic_outlook_json = economic_outlook.as_ref().map(|p| {
        json!({
            "forecastYear": c.p,
            "gdpGrowthPct": c.p,
            "inflationPct": c.p,
    ...
    Ok(Json(json!({
        "treasury_id": citizen.treasury_id,
        "budget": budget_json,
        "economic_outlook": economic_outlook_json,
    })))
}
