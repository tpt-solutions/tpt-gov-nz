//! Cross-department consent verification for the The Treasury department node.
//!
//! Mirrors `policies/treasury.rego` so the same allow/deny decision can be made locally
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "treasury"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(TreasuryError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(TreasuryError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn treasury_grant_allows_treasury_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["treasury:budget".into()]);
        let scopes = vec!["treasury:budget".into()];
