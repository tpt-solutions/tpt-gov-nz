//! Cross-department consent verification for the Ministry for Culture and Heritage department node.
//!
//! Mirrors `policies/mch.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::MchError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(MchError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MchError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn mch_grant_allows_mch_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["mch:heritage-sites".into()]);
        let scopes = vec!["mch:heritage-sites".into()];
