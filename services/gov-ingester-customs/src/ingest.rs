...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.traveller_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        if let Some(travel) = &t.travel {
            count(
                db::upsert_travel(pool, citizen_id, travel).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for declaration in &t.declarations {
            count(
                db::upsert_declaration(pool, citizen_id, declaration).await?,
                &mut inserted,
                &mut updated,
            );
        }
