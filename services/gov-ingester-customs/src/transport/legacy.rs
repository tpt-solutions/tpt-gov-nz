...
/// Connection details for the real New Zealand Customs Service legacy system integration.
///
/// In production the ingester connects to the Customs legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawCustomsBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Customs legacy transport not yet wired up (target: {}). \
