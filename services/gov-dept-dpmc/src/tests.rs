//! Integration + unit tests for the Department of the Prime Minister and Cabinet department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://dpmc:dpmc_dev_password@localhost:5432/dpmc cargo test -p gov-dept-dpmc
//! ```
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.dpmc_id, "DPMC-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["dpmc_id"], "DPMC-100001");
    assert!(data["honours"].is_null());
    assert!(data["engagements"].is_null());
}

#[sqlx::test]
async fn fetch_data_honours_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["dpmc:honours"]).await;
    let arr = data["honours"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["awardYear"], 2025);
}

#[sqlx::test]
async fn fetch_data_engagements_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["dpmc:engagements"]).await;
    let arr = data["engagements"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["eventName"], "Citizens' Honours Reception");
}

// ── actions ───────────────────────────────────────────────────────────────────────

#[sqlx::test]
async fn action_requestHonoursUpdate_valid(pool: PgPool) {
    let resp = crate::actions::execute(
        &pool,
        citizen_id(&pool).await,
        "request-honours-update",
...
#[sqlx::test]
async fn action_requestHonoursUpdate_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, DpmcError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["dpmc:honours","dpmc:engagements"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["dpmc_id"], "DPMC-100001");
    assert_eq!(body["honours"].as_array().unwrap().len(), 1);
