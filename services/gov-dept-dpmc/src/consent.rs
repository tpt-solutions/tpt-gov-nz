//! Cross-department consent verification for the Department of the Prime Minister and Cabinet department node.
//!
//! Mirrors `policies/dpmc.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::DpmcError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(DpmcError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(DpmcError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn dpmc_grant_allows_dpmc_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["dpmc:honours".into()]);
        let scopes = vec!["dpmc:honours".into()];
