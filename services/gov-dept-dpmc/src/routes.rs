...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "dpmc" }))
}
...
) -> Result<Json<ResolveResponse>, DpmcError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(DpmcError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.dpmc_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(DpmcError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let honours = if has_scope("dpmc:honours") {
        Some(db::fetch_honours(&pool, citizen.id).await?)
    } else {
        None
    };

    let honours_json = honours.as_ref().map(|rows| {
    ...
    let engagements = if has_scope("dpmc:engagements") {
    ...
    let engagements_json = engagements.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "eventName": c.c,
            "eventDate": c.c.to_string(),
            "location": c.c,
        })).collect::<Vec<_>>())
    });

    Ok(Json(json!({
        "dpmc_id": citizen.dpmc_id,
        "honours": honours_json,
