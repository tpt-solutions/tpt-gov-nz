//! Cross-department consent verification for the Ministry for Regulation department node.
//!
//! Mirrors `policies/regulation.rego` so the same allow/deny decision can be made locally
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "regulation"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(RegulationError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(RegulationError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn regulation_grant_allows_regulation_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["regulation:regulatory-reviews".into()]);
        let scopes = vec!["regulation:regulatory-reviews".into()];
