//! Integration + unit tests for the Ministry for Ethnic Communities department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://ethnic:ethnic_dev_password@localhost:5432/ethnic cargo test -p gov-dept-ethnic
//! ```
...
use crate::{build_app, EthnicError};
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.ethnic_id, "ETH-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["ethnic_id"], "ETH-100001");
    assert!(data["programmes"].is_null());
    assert!(data["community_grants"].is_null());
}

#[sqlx::test]
async fn fetch_data_programmes_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["ethnic:programmes"]).await;
    let arr = data["programmes"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["programmeName"], "Ethnic Communities Graduate Programme");
}

#[sqlx::test]
async fn fetch_data_community_grants_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["ethnic:community-grants"]).await;
    let arr = data["community_grants"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["grantName"], "Community-led response fund");
}
...
async fn action_requestGrantInfo_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, EthnicError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["ethnic:programmes","ethnic:community-grants"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["ethnic_id"], "ETH-100001");
    assert_eq!(body["programmes"].as_array().unwrap().len(), 1);
    assert_eq!(body["community_grants"].as_array().unwrap().len(), 1);
}
