//! Cross-department consent verification for the Ministry for Ethnic Communities department node.
//!
//! Mirrors `policies/ethnic.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::EthnicError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "ethnic"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(EthnicError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(EthnicError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn ethnic_grant_allows_ethnic_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["ethnic:programmes".into()]);
        let scopes = vec!["ethnic:programmes".into()];
