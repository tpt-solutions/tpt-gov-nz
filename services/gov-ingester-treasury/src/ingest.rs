...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.treasury_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for budget in &t.budget {
            count(
                db::upsert_budget(pool, citizen_id, budget).await?,
                &mut inserted,
                &mut updated,
            );
        }

        if let Some(economic_outlook) = &t.economic_outlook {
            count(
                db::upsert_economic_outlook(pool, citizen_id, economic_outlook).await?,
                &mut inserted,
                &mut updated,
            );
        }
