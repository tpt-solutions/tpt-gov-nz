//! Cross-department consent verification for the Retirement Commission (Te Ara Ahunga Ora) department node.
//!
//! Mirrors `policies/retirement.rego` so the same allow/deny decision can be made locally
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "retirement"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(RetirementError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(RetirementError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn retirement_grant_allows_retirement_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["retirement:retirement-plan".into()]);
        let scopes = vec!["retirement:retirement-plan".into()];
