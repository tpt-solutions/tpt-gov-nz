...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "retirement" }))
}
...
) -> Result<Json<ResolveResponse>, RetirementError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(RetirementError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.retirement_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(RetirementError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let retirement_plan = if has_scope("retirement:retirement-plan") {
        Some(db::fetch_retirement_plan(&pool, citizen.id).await?)
    } else {
        None
    };

    let retirement_plan_json = retirement_plan.as_ref().map(|p| {
        json!({
            "hasPlan": c.p,
            "retirementAge": c.p,
            "lastReview": c.p.to_string(),
        })
    });

    let guidance = if has_scope("retirement:guidance") {
        Some(db::fetch_guidance(&pool, citizen.id).await?)
    } else {
        None
    };

    let guidance_json = guidance.as_ref().map(|rows| {
    ...
    Ok(Json(json!({
        "retirement_id": citizen.retirement_id,
        "retirement_plan": retirement_plan_json,
