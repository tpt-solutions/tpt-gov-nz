...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.client_number).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for application in &t.applications {
            count(
                db::upsert_application(pool, citizen_id, application).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for tenancy in &t.tenancies {
            count(
                db::upsert_tenancy(pool, citizen_id, tenancy).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for request in &t.maintenance_requests {
            count(
                db::upsert_maintenance_request(pool, citizen_id, request).await?,
                &mut inserted,
                &mut updated,
            );
        }
