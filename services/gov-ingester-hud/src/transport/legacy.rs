...
/// Connection details for the real HUD / Kainga Ora legacy system integration.
///
/// In production the ingester connects to the HUD legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawHudBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "HUD legacy transport not yet wired up (target: {}). \
