//! Cross-department consent verification for the DIA department node.
//!
//! Mirrors `policies/dia.rego` so the same allow/deny decision can be made locally
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(DiaError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(DiaError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn dia_grant_allows_dia_scope() {
        let i = issuer();
        let g = grant(&i, "dia", vec!["dia:passport".into()]);
        let scopes = vec!["dia:passport".into()];
        let input = ConsentInput {
            requesting_dept_id: "dia",
