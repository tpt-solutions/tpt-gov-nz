...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.moe_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        if let Some(enrolment) = &t.enrolment {
            count(
                db::upsert_enrolment(pool, citizen_id, enrolment).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for student_support in &t.student_support {
            count(
                db::upsert_student_support(pool, citizen_id, student_support).await?,
                &mut inserted,
                &mut updated,
            );
        }
