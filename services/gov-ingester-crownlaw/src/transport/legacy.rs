...
use crate::{
    error::IngestError,
    raw::RawCrownlawBatch,
    transport::IngesterTransport,
};

/// Connection details for the real Crown Law Office legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawCrownlawBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Crown Law Office legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
