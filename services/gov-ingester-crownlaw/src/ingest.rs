...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.crownlaw_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for legal_opinions in &t.legal_opinions {
            count(
                db::upsert_legal_opinions(pool, citizen_id, legal_opinions).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for litigation in &t.litigation {
            count(
                db::upsert_litigation(pool, citizen_id, litigation).await?,
                &mut inserted,
                &mut updated,
            );
        }
