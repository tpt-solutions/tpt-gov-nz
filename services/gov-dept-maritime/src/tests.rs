//! Integration + unit tests for the Maritime New Zealand department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://maritime:maritime_dev_password@localhost:5432/maritime cargo test -p gov-dept-maritime
//! ```
...
use crate::{build_app, MaritimeError};
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.maritime_id, "MAR-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["maritime_id"], "MAR-100001");
    assert!(data["vessels"].is_null());
    assert!(data["incidents"].is_null());
}

#[sqlx::test]
async fn fetch_data_vessels_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["maritime:vessels"]).await;
    let arr = data["vessels"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["vesselName"], "MV Tane Moana");
}

#[sqlx::test]
async fn fetch_data_incidents_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["maritime:incidents"]).await;
    let arr = data["incidents"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["reference"], "MAR-2026-02");
}

// ── actions ───────────────────────────────────────────────────────────────────────

#[sqlx::test]
async fn action_reportIncident_valid(pool: PgPool) {
    let resp = crate::actions::execute(
        &pool,
        citizen_id(&pool).await,
        "report-incident",
        &json!({ "incidentType": "Test value" }),
        "citizen",
        None,
    )
    .await
...
#[sqlx::test]
async fn action_reportIncident_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, MaritimeError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["maritime:vessels","maritime:incidents"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["maritime_id"], "MAR-100001");
    assert_eq!(body["vessels"].as_array().unwrap().len(), 1);
