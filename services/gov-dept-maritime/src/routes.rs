...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "maritime" }))
}
...
) -> Result<Json<ResolveResponse>, MaritimeError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(MaritimeError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.maritime_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(MaritimeError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let vessels = if has_scope("maritime:vessels") {
        Some(db::fetch_vessels(&pool, citizen.id).await?)
    } else {
        None
    };

    let vessels_json = vessels.as_ref().map(|rows| {
    ...
    let incidents = if has_scope("maritime:incidents") {
    ...
    Ok(Json(json!({
        "maritime_id": citizen.maritime_id,
