//! Cross-department consent verification for the Maritime New Zealand department node.
//!
//! Mirrors `policies/maritime.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::MaritimeError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "maritime"
...
pub fn verify_access(
...
) -> Result<(), MaritimeError> {
    if !evaluate_allow(input) {
        return Err(MaritimeError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MaritimeError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn maritime_grant_allows_maritime_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["maritime:vessels".into()]);
        let scopes = vec!["maritime:vessels".into()];
