...
/// Connection details for the real DIA legacy system integration.
///
/// In production the ingester connects to the DIA legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawDiaBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "DIA legacy transport not yet wired up (target: {}). \
