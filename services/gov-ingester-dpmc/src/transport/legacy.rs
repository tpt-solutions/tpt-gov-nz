...
/// Connection details for the real Department of the Prime Minister and Cabinet legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawDpmcBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Department of the Prime Minister and Cabinet legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
