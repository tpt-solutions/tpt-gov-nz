...
/// Connection details for the real Education Review Office legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawEroBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Education Review Office legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
