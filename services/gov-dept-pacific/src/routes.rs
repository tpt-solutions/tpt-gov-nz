...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "pacific" }))
}
...
) -> Result<Json<ResolveResponse>, PacificError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(PacificError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.pacific_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(PacificError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let programmes = if has_scope("pacific:programmes") {
    ...
    let language_services = if has_scope("pacific:language-services") {
        Some(db::fetch_language_services(&pool, citizen.id).await?)
    } else {
        None
    };

    let language_services_json = language_services.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "service": c.c,
            "region": c.c,
            "status": c.c,
    ...
    Ok(Json(json!({
        "pacific_id": citizen.pacific_id,
        "programmes": programmes_json,
        "language_services": language_services_json,
    })))
}
