//! Cross-department consent verification for the Ministry for Pacific Peoples department node.
//!
//! Mirrors `policies/pacific.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::PacificError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "pacific"
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(PacificError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(PacificError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn pacific_grant_allows_pacific_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["pacific:programmes".into()]);
        let scopes = vec!["pacific:programmes".into()];
