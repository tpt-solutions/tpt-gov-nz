...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "corrections" }))
}
...
) -> Result<Json<ResolveResponse>, CorrectionsError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(CorrectionsError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.corrections_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
...
) -> Result<Json<Value>, CorrectionsError> {
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(CorrectionsError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let probation = if has_scope("corrections:probation") {
        Some(db::fetch_probation(&pool, citizen.id).await?)
    } else {
        None
    };

    let case = if has_scope("corrections:case") {
        Some(db::fetch_cases(&pool, citizen.id).await?)
    } else {
        None
    };

    let probation_json = probation.and_then(|p| p).map(|p| {
        json!({
            "status": p.status,
            "officerName": p.officer_name,
            "nextReportDate": p.next_report_date.to_string(),
            "location": p.location,
        })
    });

    let case_json = case.and_then(|rows| {
        if rows.is_empty() {
            None
        } else {
            Some(json!(rows.iter().map(|c| json!({
                "caseNumber": c.case_number,
                "sentenceType": c.sentence_type,
                "startDate": c.start_date.to_string(),
                "endDate": c.end_date.map(|d| d.to_string()),
                "summary": c.summary,
            })).collect::<Vec<_>>()))
        }
    });
    ...
    Ok(Json(json!({
        "correctionsId": citizen.corrections_id,
        "probation": probation_json,
        "case": case_json,
    })))
}
