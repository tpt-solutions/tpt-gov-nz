...
/// Connection details for the real NZTA legacy system integration.
///
/// In production the ingester connects to the NZTA legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawNztaBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "NZTA legacy transport not yet wired up (target: {}). \
