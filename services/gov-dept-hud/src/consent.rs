//! Cross-department consent verification for the HUD department node.
//!
//! Mirrors `policies/hud.rego` so the same allow/deny decision can be made locally
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(HudError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(HudError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn hud_grant_allows_hud_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["hud:applications".into()]);
        let scopes = vec!["hud:applications".into()];
