//! Cross-department consent verification for the MOJ department node.
//!
//! Mirrors `policies/moj.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::MojError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(MojError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MojError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn moj_grant_allows_moj_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["moj:fines".into()]);
        let scopes = vec!["moj:fines".into()];
