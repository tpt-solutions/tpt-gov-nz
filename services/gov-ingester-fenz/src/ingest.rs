...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.fenz_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        if let Some(fire_safety) = &t.fire_safety {
            count(
                db::upsert_fire_safety(pool, citizen_id, fire_safety).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for incidents in &t.incidents {
            count(
                db::upsert_incidents(pool, citizen_id, incidents).await?,
                &mut inserted,
                &mut updated,
            );
        }
