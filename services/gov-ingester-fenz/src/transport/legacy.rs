...
/// Connection details for the real Fire and Emergency New Zealand legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawFenzBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Fire and Emergency New Zealand legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
