//! Cross-department consent verification for the NZTA department node.
//!
//! Mirrors `policies/nzta.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::NztaError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(NztaError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(NztaError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    fn grant(issuer: &CredentialIssuer, requesting: &str, scopes: Vec<String>) -> DataGrantCredential {
    ...
    #[test]
    fn nzta_grant_allows_nzta_scope() {
        let i = issuer();
        let g = grant(&i, "nzta", vec!["nzta:driver-licence".into()]);
        let scopes = vec!["nzta:driver-licence".into()];
        let input = ConsentInput {
            requesting_dept_id: "nzta",
