...
/// Connection details for the real New Zealand Security Intelligence Service legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawNzsisBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "New Zealand Security Intelligence Service legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
