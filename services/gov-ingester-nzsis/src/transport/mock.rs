...
/// Reads a raw New Zealand Security Intelligence Service batch from a JSON fixture file on disk (dev/demo/CI).
...
impl IngesterTransport for MockTransport {
    async fn pull(&self) -> Result<RawNzsisBatch, IngestError> {
        ...
        let batch = serde_json::from_slice::<RawNzsisBatch>(&bytes)
