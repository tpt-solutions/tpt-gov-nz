...
use crate::models::{InvestigationsEntity,OutcomesEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, sfo_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET sfo_id = EXCLUDED.sfo_id
...
pub async fn upsert_investigations(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO sfo_investigations (citizen_id, reference, matter, status, opened_date)
           VALUES ($1, $2, $3, $4, $5)
...
pub async fn upsert_outcomes(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO sfo_outcomes (citizen_id, reference, result, result_date)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (citizen_id, reference) DO UPDATE SET
              reference = EXCLUDED.reference, result = EXCLUDED.result, result_date = EXCLUDED.result_date
           RETURNING (xmax = 0) AS inserted"#,
        citizen_id,
        e.reference,
        e.result,
