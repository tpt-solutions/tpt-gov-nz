...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.customer_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for title in &t.titles {
            count(
                db::upsert_title(pool, citizen_id, title).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for ownership in &t.ownership {
            count(
                db::upsert_ownership(pool, citizen_id, ownership).await?,
                &mut inserted,
                &mut updated,
            );
        }
