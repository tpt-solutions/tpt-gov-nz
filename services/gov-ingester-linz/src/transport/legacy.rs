...
/// Connection details for the real LINZ legacy system integration.
///
/// In production the ingester connects to the LINZ legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawLinzBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "LINZ legacy transport not yet wired up (target: {}). \
