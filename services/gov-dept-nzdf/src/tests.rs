//! Integration + unit tests for the New Zealand Defence Force department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://nzdf:nzdf_dev_password@localhost:5432/nzdf cargo test -p gov-dept-nzdf
//! ```
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.nzdf_id, "NZDF-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["nzdf_id"], "NZDF-100001");
    assert!(data["service_records"].is_null());
    assert!(data["deployments"].is_null());
}

#[sqlx::test]
async fn fetch_data_service_records_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["nzdf:service-records"]).await;
    let arr = data["service_records"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["serviceNo"], "NZDF-55821");
}

#[sqlx::test]
async fn fetch_data_deployments_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["nzdf:deployments"]).await;
    let arr = data["deployments"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["operation"], "Burnham readiness");
}

// ── actions ───────────────────────────────────────────────────────────────────────

#[sqlx::test]
async fn action_requestServiceRecord_valid(pool: PgPool) {
    let resp = crate::actions::execute(
        &pool,
        citizen_id(&pool).await,
        "request-service-record",
        &json!({ "serviceNo": "Test value" }),
        "citizen",
        None,
    )
    .await
...
#[sqlx::test]
async fn action_requestServiceRecord_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, NzdfError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["nzdf:service-records","nzdf:deployments"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["nzdf_id"], "NZDF-100001");
    assert_eq!(body["service_records"].as_array().unwrap().len(), 1);
