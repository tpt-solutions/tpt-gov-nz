//! Cross-department consent verification for the New Zealand Defence Force department node.
//!
//! Mirrors `policies/nzdf.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::NzdfError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(NzdfError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(NzdfError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn nzdf_grant_allows_nzdf_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["nzdf:service-records".into()]);
        let scopes = vec!["nzdf:service-records".into()];
