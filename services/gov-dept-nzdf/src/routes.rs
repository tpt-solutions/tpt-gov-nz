...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "nzdf" }))
}
...
) -> Result<Json<ResolveResponse>, NzdfError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(NzdfError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.nzdf_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(NzdfError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let service_records = if has_scope("nzdf:service-records") {
        Some(db::fetch_service_records(&pool, citizen.id).await?)
    } else {
        None
    };

    let service_records_json = service_records.as_ref().map(|rows| {
    ...
    let deployments = if has_scope("nzdf:deployments") {
        Some(db::fetch_deployments(&pool, citizen.id).await?)
    } else {
        None
    };

    let deployments_json = deployments.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "operation": c.c,
            "country": c.c,
            "year": c.c,
    ...
    Ok(Json(json!({
        "nzdf_id": citizen.nzdf_id,
        "service_records": service_records_json,
