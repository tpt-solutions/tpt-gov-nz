//! Cross-department consent verification for the Crown Law Office department node.
//!
//! Mirrors `policies/crownlaw.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::CrownlawError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "crownlaw"
...
pub fn verify_access(
...
) -> Result<(), CrownlawError> {
    if !evaluate_allow(input) {
        return Err(CrownlawError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(CrownlawError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn crownlaw_grant_allows_crownlaw_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["crownlaw:legal-opinions".into()]);
        let scopes = vec!["crownlaw:legal-opinions".into()];
