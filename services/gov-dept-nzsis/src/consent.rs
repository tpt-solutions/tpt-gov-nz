//! Cross-department consent verification for the New Zealand Security Intelligence Service department node.
//!
//! Mirrors `policies/nzsis.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::NzsisError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(NzsisError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(NzsisError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn nzsis_grant_allows_nzsis_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["nzsis:mandates".into()]);
        let scopes = vec!["nzsis:mandates".into()];
