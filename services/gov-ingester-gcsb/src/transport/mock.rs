...
/// Reads a raw Government Communications Security Bureau batch from a JSON fixture file on disk (dev/demo/CI).
...
impl IngesterTransport for MockTransport {
    async fn pull(&self) -> Result<RawGcsbBatch, IngestError> {
        ...
        let batch = serde_json::from_slice::<RawGcsbBatch>(&bytes)
