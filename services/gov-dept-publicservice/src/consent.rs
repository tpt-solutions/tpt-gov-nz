//! Cross-department consent verification for the Te Kawa Mataaho Public Service Commission department node.
//!
//! Mirrors `policies/publicservice.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::PublicserviceError;
...
fn scope_covered(input: &ConsentInput, scope: &str) -> bool {
    ...
    input.consent_grants.iter().any(|grant| {
        grant.requesting_dept_id == input.requesting_dept_id
            && grant.providing_dept_id == "publicservice"
...
pub fn verify_access(
...
) -> Result<(), PublicserviceError> {
    if !evaluate_allow(input) {
        return Err(PublicserviceError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(PublicserviceError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn publicservice_grant_allows_publicservice_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["publicservice:workforce".into()]);
        let scopes = vec!["publicservice:workforce".into()];
