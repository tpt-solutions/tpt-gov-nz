...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.retirement_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        if let Some(retirement_plan) = &t.retirement_plan {
            count(
                db::upsert_retirement_plan(pool, citizen_id, retirement_plan).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for guidance in &t.guidance {
            count(
                db::upsert_guidance(pool, citizen_id, guidance).await?,
                &mut inserted,
                &mut updated,
            );
        }
