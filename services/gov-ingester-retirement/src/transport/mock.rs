...
/// Reads a raw Retirement Commission (Te Ara Ahunga Ora) batch from a JSON fixture file on disk (dev/demo/CI).
...
impl IngesterTransport for MockTransport {
    async fn pull(&self) -> Result<RawRetirementBatch, IngestError> {
        ...
        let batch = serde_json::from_slice::<RawRetirementBatch>(&bytes)
