...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.nzdf_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for service_records in &t.service_records {
            count(
                db::upsert_service_records(pool, citizen_id, service_records).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for deployments in &t.deployments {
            count(
                db::upsert_deployments(pool, citizen_id, deployments).await?,
                &mut inserted,
                &mut updated,
            );
        }
