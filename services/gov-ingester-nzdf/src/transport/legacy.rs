...
/// Connection details for the real New Zealand Defence Force legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawNzdfBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "New Zealand Defence Force legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
