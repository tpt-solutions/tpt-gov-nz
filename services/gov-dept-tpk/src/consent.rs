//! Cross-department consent verification for the TPK department node.
//!
//! Mirrors `policies/tpk.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::TpkError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(TpkError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(TpkError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn tpk_grant_allows_tpk_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["tpk:programmes".into()]);
        let scopes = vec!["tpk:programmes".into()];
