...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "tpk" }))
}
...
) -> Result<Json<ResolveResponse>, TpkError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(TpkError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.tpk_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(TpkError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let programmes = if has_scope("tpk:programmes") {
        Some(db::fetch_programmes(&pool, citizen.id).await?)
    } else {
        None
    };

    let funding = if has_scope("tpk:funding") {
