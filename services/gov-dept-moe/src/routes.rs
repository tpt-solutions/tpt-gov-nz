...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "moe" }))
}
...
) -> Result<Json<ResolveResponse>, MoeError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(MoeError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.moe_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(MoeError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let enrolment = if has_scope("moe:enrolment") {
        Some(db::fetch_enrolment(&pool, citizen.id).await?)
    } else {
        None
    };

    let enrolment_json = enrolment.as_ref().map(|p| {
        json!({
            "school": c.p,
            "yearLevel": c.p,
            "status": c.p,
    ...
    let student_support = if has_scope("moe:student-support") {
        Some(db::fetch_student_support(&pool, citizen.id).await?)
    } else {
        None
    };

    let student_support_json = student_support.as_ref().map(|rows| {
    ...
    Ok(Json(json!({
        "moe_id": citizen.moe_id,
        "enrolment": enrolment_json,
        "student_support": student_support_json,
    })))
}
