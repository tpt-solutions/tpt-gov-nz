...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "gcsb" }))
}
...
) -> Result<Json<ResolveResponse>, GcsbError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(GcsbError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.gcsb_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(GcsbError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let mandates = if has_scope("gcsb:mandates") {
    ...
    let engagements = if has_scope("gcsb:engagements") {
