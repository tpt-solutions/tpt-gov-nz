//! Integration + unit tests for the Government Communications Security Bureau department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://gcsb:gcsb_dev_password@localhost:5432/gcsb cargo test -p gov-dept-gcsb
//! ```
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.gcsb_id, "GCSB-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["gcsb_id"], "GCSB-100001");
...
async fn fetch_data_mandates_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["gcsb:mandates"]).await;
    let arr = data["mandates"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["reference"], "GCSB-M-2026-001");
}

#[sqlx::test]
async fn fetch_data_engagements_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["gcsb:engagements"]).await;
    let arr = data["engagements"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["partner"], "CERT NZ");
}
...
async fn action_requestMandateInfo_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, GcsbError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["gcsb:mandates","gcsb:engagements"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["gcsb_id"], "GCSB-100001");
