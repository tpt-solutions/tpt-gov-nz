//! Cross-department consent verification for the Government Communications Security Bureau department node.
//!
//! Mirrors `policies/gcsb.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::GcsbError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(GcsbError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(GcsbError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn gcsb_grant_allows_gcsb_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["gcsb:mandates".into()]);
        let scopes = vec!["gcsb:mandates".into()];
