...
pub async fn execute(
    ...
    let result = match action_type {
        ...
        _ => Err(GcsbError::InvalidAction(format!(
...
async fn requestMandateInfo(
    ...
    _citizen_id: Uuid,
    parameters: &Value,
) -> Result<Value, GcsbError> {
    let reference = parameters
        .get("reference")
        .and_then(|v| v.as_str())
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
        .ok_or_else(|| GcsbError::InvalidAction("reference must not be empty".into()))?;
