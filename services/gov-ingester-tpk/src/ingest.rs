...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.tpk_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for programme in &t.programmes {
            count(
                db::upsert_programme(pool, citizen_id, programme).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for funding in &t.funding {
            count(
                db::upsert_funding(pool, citizen_id, funding).await?,
                &mut inserted,
                &mut updated,
            );
        }
