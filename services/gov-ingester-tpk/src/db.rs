...
use crate::models::{FundingEntity, IngestionStatus, ProgrammeEntity};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, tpk_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET tpk_id = EXCLUDED.tpk_id
...
pub async fn upsert_programme(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &ProgrammeEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO tpk_programmes (citizen_id, programme_name, status, region)
           VALUES ($1, $2, $3, $4)
...
pub async fn upsert_funding(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO tpk_funding (citizen_id, grant_id, amount, purpose, status)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (citizen_id, grant_id) DO UPDATE SET
