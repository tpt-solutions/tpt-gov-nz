...
/// Connection details for the real Te Puni Kōkiri legacy system integration.
///
/// In production the ingester connects to the TPK legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawTpkBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "TPK legacy transport not yet wired up (target: {}). \
