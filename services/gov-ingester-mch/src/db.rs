...
use crate::models::{HeritageSitesEntity,GrantsEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, mch_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET mch_id = EXCLUDED.mch_id
...
pub async fn upsert_heritage_sites(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &HeritageSitesEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO mch_heritage_sites (citizen_id, name, status, region)
           VALUES ($1, $2, $3, $4)
...
pub async fn upsert_grants(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &GrantsEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO mch_grants (citizen_id, grant_name, amount, status)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (citizen_id, grant_name) DO UPDATE SET
              grant_name = EXCLUDED.grant_name, amount = EXCLUDED.amount, status = EXCLUDED.status
           RETURNING (xmax = 0) AS inserted"#,
        citizen_id,
        e.grant_name,
        e.amount,
