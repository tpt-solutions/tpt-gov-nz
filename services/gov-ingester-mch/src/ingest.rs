...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.mch_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for heritage_sites in &t.heritage_sites {
            count(
                db::upsert_heritage_sites(pool, citizen_id, heritage_sites).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for grants in &t.grants {
            count(
                db::upsert_grants(pool, citizen_id, grants).await?,
                &mut inserted,
                &mut updated,
            );
        }
