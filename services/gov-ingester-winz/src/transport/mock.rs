...
/// Reads a raw WINZ batch from a JSON fixture file on disk.
///
/// Used in dev/demo and in CI so the full ingestion pipeline can run with no
/// legacy WINZ system attached. Point `WINZ_MOCK_FIXTURE` at any `RawWinzBatch` JSON.
...
impl IngesterTransport for MockTransport {
    async fn pull(&self) -> Result<RawWinzBatch, IngestError> {
        ...
        let batch = serde_json::from_slice::<RawWinzBatch>(&bytes)
