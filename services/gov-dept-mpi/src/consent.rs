//! Cross-department consent verification for the MPI department node.
//!
//! Mirrors `policies/mpi.rego` so the same allow/deny decision can be made locally
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(MpiError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(MpiError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn mpi_grant_allows_mpi_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["mpi:registrations".into()]);
        let scopes = vec!["mpi:registrations".into()];
