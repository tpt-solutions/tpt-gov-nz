...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.defence_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for procurements in &t.procurements {
            count(
                db::upsert_procurements(pool, citizen_id, procurements).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for bases in &t.bases {
            count(
                db::upsert_bases(pool, citizen_id, bases).await?,
                &mut inserted,
                &mut updated,
            );
        }
