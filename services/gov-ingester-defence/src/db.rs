...
use crate::models::{ProcurementsEntity,BasesEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, defence_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET defence_id = EXCLUDED.defence_id
...
pub async fn upsert_procurements(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &ProcurementsEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO defence_procurements (citizen_id, programme, value, status)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (citizen_id, programme) DO UPDATE SET
              programme = EXCLUDED.programme, value = EXCLUDED.value, status = EXCLUDED.status
...
pub async fn upsert_bases(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &BasesEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO defence_bases (citizen_id, name, location, status)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (citizen_id, name) DO UPDATE SET
