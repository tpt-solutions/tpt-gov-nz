...
/// Connection details for the real MPI legacy system integration.
///
/// In production the ingester connects to the MPI legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMpiBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "MPI legacy transport not yet wired up (target: {}). \
