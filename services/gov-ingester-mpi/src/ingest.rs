...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.mpi_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for registration in &t.registrations {
            count(
                db::upsert_registration(pool, citizen_id, registration).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for certification in &t.certifications {
            count(
                db::upsert_certification(pool, citizen_id, certification).await?,
                &mut inserted,
                &mut updated,
            );
        }
