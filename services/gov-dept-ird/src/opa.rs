...
//!
//! When the IRD node is deployed with an OPA sidecar (`OPA_URL` set) and built with
//! the `opa` feature, the route layer defers the consent decision to the deployed
//! `policies/ird.rego`. The local [`crate::consent`] evaluation is always available
...
/// Query the OPA sidecar for the `ird.consent` decision.
///
