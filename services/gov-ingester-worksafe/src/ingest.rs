...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.worksafe_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for inspections in &t.inspections {
            count(
                db::upsert_inspections(pool, citizen_id, inspections).await?,
                &mut inserted,
                &mut updated,
            );
        }

        for investigations in &t.investigations {
            count(
                db::upsert_investigations(pool, citizen_id, investigations).await?,
                &mut inserted,
                &mut updated,
            );
        }
