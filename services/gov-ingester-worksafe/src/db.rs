...
use crate::models::{InspectionsEntity,InvestigationsEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, worksafe_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET worksafe_id = EXCLUDED.worksafe_id
...
pub async fn upsert_inspections(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO worksafe_inspections (citizen_id, reference, site, inspection_date, outcome)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (citizen_id, reference) DO UPDATE SET
              reference = EXCLUDED.reference, site = EXCLUDED.site, inspection_date = EXCLUDED.inspection_date, outcome = EXCLUDED.outcome
...
pub async fn upsert_investigations(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &InvestigationsEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO worksafe_investigations (citizen_id, reference, matter, status, opened_date)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (citizen_id, reference) DO UPDATE SET
              reference = EXCLUDED.reference, matter = EXCLUDED.matter, status = EXCLUDED.status, opened_date = EXCLUDED.opened_date
           RETURNING (xmax = 0) AS inserted"#,
        citizen_id,
        e.reference,
        e.matter,
