...
use crate::{
    error::IngestError,
    raw::RawWorksafeBatch,
    transport::IngesterTransport,
};

/// Connection details for the real WorkSafe New Zealand legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawWorksafeBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "WorkSafe New Zealand legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
