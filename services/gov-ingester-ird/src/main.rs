...
use gov_ingester_ird::{
    config::{Config, TransportKind},
    ingest,
    transport::{mock::MockTransport, sftp::SftpTransport, IngesterTransport},
    IngestError,
};
use sqlx::PgPool;
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("gov_ingester_ird=info,sqlx=warn")),
        )
        .json()
        .init();
    ...
    // Ensure the department schema exists (the ingester owns writing to the dept DB).
    sqlx::migrate!("../gov-dept-ird/migrations")
        .run(&pool)
        .await?;
    ...
    let transport: Box<dyn IngesterTransport> = match config.transport {
        ...
        TransportKind::Sftp => Box::new(SftpTransport::new(config.sftp.clone())),
    };
