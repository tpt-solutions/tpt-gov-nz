//! Cross-department consent verification for the Earthquake Commission (Toka Tū Ake) department node.
//!
//! Mirrors `policies/eqc.rego` so the same allow/deny decision can be made locally
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(EqcError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(EqcError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn eqc_grant_allows_eqc_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["eqc:claims".into()]);
        let scopes = vec!["eqc:claims".into()];
