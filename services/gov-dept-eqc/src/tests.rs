//! Integration + unit tests for the Earthquake Commission (Toka Tū Ake) department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://eqc:eqc_dev_password@localhost:5432/eqc cargo test -p gov-dept-eqc
//! ```
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.eqc_id, "EQC-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["eqc_id"], "EQC-100001");
...
async fn fetch_data_claims_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["eqc:claims"]).await;
    let arr = data["claims"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["reference"], "EQC-2026-007");
}

#[sqlx::test]
async fn fetch_data_cover_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["eqc:cover"]).await;
    let obj = data["cover"].as_object().unwrap();
...
#[sqlx::test]
async fn action_requestClaimUpdate_valid(pool: PgPool) {
...
#[sqlx::test]
async fn action_requestClaimUpdate_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, EqcError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["eqc:claims","eqc:cover"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["eqc_id"], "EQC-100001");
    assert_eq!(body["claims"].as_array().unwrap().len(), 1);
    assert!(body["cover"].as_object().is_some());
}
...
#[sqlx::test]
async fn cross_dept_requests_eqc_with_valid_grant(pool: PgPool) {
