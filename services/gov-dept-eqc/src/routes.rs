...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "eqc" }))
}
...
) -> Result<Json<ResolveResponse>, EqcError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(EqcError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.eqc_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(EqcError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let claims = if has_scope("eqc:claims") {
    ...
    let claims_json = claims.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            ...
            "property": c.c,
            "status": c.c,
            "lodgedDate": c.c.to_string(),
        })).collect::<Vec<_>>())
    });

    let cover = if has_scope("eqc:cover") {
        Some(db::fetch_cover(&pool, citizen.id).await?)
    } else {
        None
    };

    let cover_json = cover.as_ref().map(|p| {
        json!({
            "property": c.p,
            "sumInsured": c.p,
            "validTo": c.p.to_string(),
        })
    });

    Ok(Json(json!({
        "eqc_id": citizen.eqc_id,
        "claims": claims_json,
        "cover": cover_json,
    })))
}
