...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "doc" }))
}
...
) -> Result<Json<ResolveResponse>, DocError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(DocError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.doc_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(DocError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let permits = if has_scope("doc:permits") {
        Some(db::fetch_permits(&pool, citizen.id).await?)
    } else {
        None
    };

    let concessions = if has_scope("doc:concessions") {
        Some(db::fetch_concessions(&pool, citizen.id).await?)
    } else {
        None
    };

    let permits_json = permits.as_ref().map(|rows| {
        json!(rows.iter().map(|p| json!({
            "permitNumber": p.permit_number,
            "activity": p.activity,
            "location": p.location,
            "status": p.status,
            "expiresDate": p.expires_date.to_string(),
        })).collect::<Vec<_>>())
    });

    let concessions_json = concessions.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "concessionId": c.concession_id,
            "type": c.r#type,
            "holder": c.holder,
    ...
    Ok(Json(json!({
        "docId": citizen.doc_id,
        "permits": permits_json,
        "concessions": concessions_json,
    })))
}
