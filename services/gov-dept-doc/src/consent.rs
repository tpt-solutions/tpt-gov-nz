//! Cross-department consent verification for the DOC department node.
//!
//! Mirrors `policies/doc.rego` so the same allow/deny decision can be made locally
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(DocError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(DocError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn doc_grant_allows_doc_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["doc:permits".into()]);
        let scopes = vec!["doc:permits".into()];
