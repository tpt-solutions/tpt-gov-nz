...
use crate::{
    error::IngestError,
    raw::RawCorrectionsBatch,
    transport::IngesterTransport,
};

/// Connection details for the real Department of Corrections legacy system integration.
///
/// In production the ingester connects to the Corrections legacy batch extract, downloads
/// the periodic batch, and parses it. This struct carries the configuration; the
/// [`IngesterTransport::pull`] implementation is intentionally a stub for Phase 1.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawCorrectionsBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Corrections legacy transport not yet wired up (target: {}). \
