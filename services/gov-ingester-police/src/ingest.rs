...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        for infringement in &t.infringements {
            count(
                db::upsert_infringement(pool, citizen_id, infringement).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for report in &t.reports {
            count(db::upsert_report(pool, citizen_id, report).await?, &mut inserted, &mut updated);
        }
    }
