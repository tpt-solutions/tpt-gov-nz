//! Cross-department consent verification for the Tertiary Education Commission department node.
//!
//! Mirrors `policies/tec.rego` so the same allow/deny decision can be made locally
//! (and in unit tests) without a running OPA sidecar.

use crate::error::TecError;
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(TecError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(TecError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn tec_grant_allows_tec_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["tec:funding".into()]);
        let scopes = vec!["tec:funding".into()];
