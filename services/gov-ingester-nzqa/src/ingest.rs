...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.nsn).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for qualification in &t.qualifications {
            count(
                db::upsert_qualification(pool, citizen_id, qualification).await?,
                &mut inserted,
                &mut updated,
            );
        }
        if let Some(transcript) = &t.transcript {
            count(
                db::upsert_transcript(pool, citizen_id, transcript).await?,
                &mut inserted,
                &mut updated,
            );
        }
