...
/// Connection details for the real NZQA / Ministry of Education legacy system integration.
///
/// In production the ingester connects to the NZQA legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawNzqaBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "NZQA legacy transport not yet wired up (target: {}). \
