...
/// Connection details for the real MOH legacy system integration (NHI / HPI).
///
/// In production the ingester connects to the MOH legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMohBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "MOH legacy transport not yet wired up (target: {}). \
