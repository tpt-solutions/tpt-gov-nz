...
pub async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "dept_id": "sfo" }))
}
...
) -> Result<Json<ResolveResponse>, SfoError> {
    let row = db::resolve_by_did(&pool, &req.did)
        .await?
        .ok_or(SfoError::CitizenNotFound)?;

    Ok(Json(ResolveResponse {
        did: row.did,
        dept_local_id: row.sfo_id,
        display_name: None,
    }))
}
...
pub async fn fetch_data(
    ...
    if !is_direct_access(&req.requesting_dept_id) {
        ...
        match opa::evaluate(&input).await {
            Some(true) => {}
            Some(false) => return Err(SfoError::ScopeNotGranted(req.scopes.join(","))),
    ...
    let investigations = if has_scope("sfo:investigations") {
    ...
    let outcomes = if has_scope("sfo:outcomes") {
        Some(db::fetch_outcomes(&pool, citizen.id).await?)
    } else {
        None
    };

    let outcomes_json = outcomes.as_ref().map(|rows| {
        json!(rows.iter().map(|c| json!({
            "reference": c.c,
            "result": c.c,
            "resultDate": c.c.to_string(),
        })).collect::<Vec<_>>())
    });

    Ok(Json(json!({
        "sfo_id": citizen.sfo_id,
