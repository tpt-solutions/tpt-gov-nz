//! Cross-department consent verification for the Serious Fraud Office department node.
//!
//! Mirrors `policies/sfo.rego` so the same allow/deny decision can be made locally
...
pub fn verify_access(
    ...
    if !evaluate_allow(input) {
        return Err(SfoError::ScopeNotGranted(denied_scopes(input).join(",")));
    }
    ...
    if let Some(key) = issuer_key {
        ...
        if !all_signed {
            return Err(SfoError::ScopeNotGranted("grant signature invalid".into()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn sfo_grant_allows_sfo_scope() {
        let i = issuer();
        let g = grant(&i, "ird", vec!["sfo:investigations".into()]);
        let scopes = vec!["sfo:investigations".into()];
