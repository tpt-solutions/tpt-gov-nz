//! Integration + unit tests for the Serious Fraud Office department service.
//!
//! Run with a Postgres available:
//!
//! ```sh
//! DATABASE_URL=postgresql://sfo:sfo_dev_password@localhost:5432/sfo cargo test -p gov-dept-sfo
//! ```
...
async fn resolve_found(pool: PgPool) {
    ...
    assert_eq!(row.sfo_id, "SFO-100001");
}
...
async fn fetch_data_no_scopes_returns_minimal(pool: PgPool) {
    ...
    assert_eq!(data["sfo_id"], "SFO-100001");
    assert!(data["investigations"].is_null());
    assert!(data["outcomes"].is_null());
}

#[sqlx::test]
async fn fetch_data_investigations_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["sfo:investigations"]).await;
    let arr = data["investigations"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["reference"], "SFO-2026-014");
}

#[sqlx::test]
async fn fetch_data_outcomes_scope(pool: PgPool) {
    let data = fetch_data(&pool, &["sfo:outcomes"]).await;
    let arr = data["outcomes"].as_array().unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["reference"], "SFO-2025-009");
}
...
async fn action_requestInvestigationUpdate_rejects_empty(pool: PgPool) {
    ...
    .await
    .unwrap_err();
    assert!(matches!(err, SfoError::InvalidAction(_)));
}
...
async fn http_round_trip(pool: PgPool) {
    ...
    let data = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(
            json!({ "did": TEST_DID, "scopes": ["sfo:investigations","sfo:outcomes"] })
                .to_string(),
        ))
        .unwrap();
    ...
    assert_eq!(body["sfo_id"], "SFO-100001");
