...
/// Connection details for the real MBIE legacy system integration.
///
/// In production the ingester connects to the MBIE legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMbieBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "MBIE legacy transport not yet wired up (target: {}). \
