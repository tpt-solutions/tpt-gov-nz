...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.person_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for business in &t.businesses {
            count(
                db::upsert_business(pool, citizen_id, business).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for directorship in &t.directorships {
            count(
                db::upsert_directorship(pool, citizen_id, directorship).await?,
                &mut inserted,
                &mut updated,
            );
        }
