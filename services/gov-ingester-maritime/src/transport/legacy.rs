...
use crate::{
    error::IngestError,
    raw::RawMaritimeBatch,
    transport::IngesterTransport,
};

/// Connection details for the real Maritime New Zealand legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMaritimeBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Maritime New Zealand legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
