...
use crate::models::{VesselsEntity,IncidentsEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, maritime_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET maritime_id = EXCLUDED.maritime_id
...
pub async fn upsert_vessels(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &VesselsEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO maritime_vessels (citizen_id, vessel_name, flag, status)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (citizen_id, vessel_name) DO UPDATE SET
              vessel_name = EXCLUDED.vessel_name, flag = EXCLUDED.flag, status = EXCLUDED.status
...
pub async fn upsert_incidents(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO maritime_incidents (citizen_id, reference, incident_type, incident_date, status)
           VALUES ($1, $2, $3, $4, $5)
