...
/// Connection details for the real Tertiary Education Commission legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawTecBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Tertiary Education Commission legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
