...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        let (citizen_id, was_inserted) =
            db::upsert_citizen(pool, &t.citizen.did, &t.citizen.ethnic_id).await?;
        if was_inserted {
            inserted += 1;
        } else {
            updated += 1;
        ...
        for community_grants in &t.community_grants {
            count(
                db::upsert_community_grants(pool, citizen_id, community_grants).await?,
                &mut inserted,
                &mut updated,
            );
        }
