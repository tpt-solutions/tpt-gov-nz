...
use crate::models::{ProgrammesEntity,CommunityGrantsEntity,IngestionStatus};
...
pub async fn upsert_citizen(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO citizens (did, ethnic_id)
           VALUES ($1, $2)
           ON CONFLICT (did) DO UPDATE SET ethnic_id = EXCLUDED.ethnic_id
...
pub async fn upsert_programmes(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO ethnic_programmes (citizen_id, programme_name, status, year)
           VALUES ($1, $2, $3, $4)
...
pub async fn upsert_community_grants(
    pool: &PgPool,
    citizen_id: Uuid,
    e: &CommunityGrantsEntity,
) -> sqlx::Result<bool> {
    let row = sqlx::query!(
        r#"INSERT INTO ethnic_community_grants (citizen_id, grant_name, amount, status)
           VALUES ($1, $2, $3, $4)
