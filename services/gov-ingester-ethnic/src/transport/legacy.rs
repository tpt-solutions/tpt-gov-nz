...
/// Connection details for the real Ministry for Ethnic Communities legacy system integration.
#[derive(Debug, Clone)]
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawEthnicBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "Ministry for Ethnic Communities legacy transport not yet wired up (target: {}).              Implement the legacy system download in production; the transform +              upsert pipeline is transport-agnostic and ready to consume the result.",
