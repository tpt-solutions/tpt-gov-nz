...
pub async fn run_once(
    ...
    for raw in &batch.citizens {
        ...
        for fine in &t.fines {
            count(db::upsert_fine(pool, citizen_id, fine).await?, &mut inserted, &mut updated);
        }
        for dispute in &t.disputes {
            count(
                db::upsert_dispute(pool, citizen_id, dispute).await?,
                &mut inserted,
                &mut updated,
            );
        }
        for record in &t.court_records {
            count(
                db::upsert_court_record(pool, citizen_id, record).await?,
                &mut inserted,
                &mut updated,
            );
        }
