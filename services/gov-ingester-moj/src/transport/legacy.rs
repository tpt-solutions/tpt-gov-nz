...
/// Connection details for the real MOJ legacy system integration.
///
/// In production the ingester connects to the MOJ legacy batch extract, downloads
...
impl IngesterTransport for LegacyTransport {
    async fn pull(&self) -> Result<RawMojBatch, IngestError> {
        Err(IngestError::NotImplemented(format!(
            "MOJ legacy transport not yet wired up (target: {}). \
