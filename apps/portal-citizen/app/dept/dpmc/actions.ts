"use server";

import type { DpmcDataBundle } from "@tpt/gov-schema";
import { askWithContext } from "../../ai/client";
import { fetchDeptData } from "../../lib/data-access";
import { getCitizenDid } from "../../lib/session";
import { produceDpmcAiContext } from "./dpmc-ai";

const DPMC_SERVICE_URL = process.env.DPMC_SERVICE_URL ?? "http://localhost:8121";

export async function fetchDpmcData(scopes: string[]): Promise<DpmcDataBundle | null> {
  return (await fetchDeptData("dpmc", scopes)) as DpmcDataBundle | null;
}

export async function submitDpmcAction(
  type: string,
  parameters: Record<string, unknown>
): Promise<{ success: boolean; message?: string }> {
  if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") {
    return { success: true, message: "Demo mode: action recorded locally (no live service)." };
  }

  const did = await getCitizenDid();
  if (!did) return { success: false, message: "Not authenticated" };

  try {
    const res = await fetch(DPMC_SERVICE_URL + "/citizen/action", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ did, type, parameters, performed_by: "citizen" }),
    });

    if (!res.ok) {
      const err = (await res.json()) as { error?: string };
      return { success: false, message: err.error ?? "Action failed" };
    }

    return res.json() as Promise<{ success: boolean; message?: string }>;
  } catch {
    return { success: false, message: "Service unavailable" };
  }
}

// ── AI ─────────────────────────────────────────────────────────────

export async function askDpmc(question: string): Promise<{ answer: string; enabled: boolean }> {
  const data = await fetchDpmcData(["dpmc:honours", "dpmc:engagements"]);
  if (!data) {
    return { answer: "Unable to load your Department of the Prime Minister and Cabinet information.", enabled: false };
  }
  const context = produceDpmcAiContext(data);
  return askWithContext(question, context);
}
