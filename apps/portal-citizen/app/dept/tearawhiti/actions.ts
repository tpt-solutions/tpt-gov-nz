"use server";

import type { TearawhitiDataBundle } from "@tpt/gov-schema";
import { askWithContext } from "../../ai/client";
import { fetchDeptData } from "../../lib/data-access";
import { getCitizenDid } from "../../lib/session";
import { produceTearawhitiAiContext } from "./tearawhiti-ai";

const TEARAWHITI_SERVICE_URL = process.env.TEARAWHITI_SERVICE_URL ?? "http://localhost:8129";

export async function fetchTearawhitiData(scopes: string[]): Promise<TearawhitiDataBundle | null> {
  return (await fetchDeptData("tearawhiti", scopes)) as TearawhitiDataBundle | null;
}

export async function submitTearawhitiAction(
  type: string,
  parameters: Record<string, unknown>
): Promise<{ success: boolean; message?: string }> {
  if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") {
    return { success: true, message: "Demo mode: action recorded locally (no live service)." };
  }

  const did = await getCitizenDid();
  if (!did) return { success: false, message: "Not authenticated" };

  try {
    const res = await fetch(TEARAWHITI_SERVICE_URL + "/citizen/action", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ did, type, parameters, performed_by: "citizen" }),
    });

    if (!res.ok) {
      const err = (await res.json()) as { error?: string };
      return { success: false, message: err.error ?? "Action failed" };
    }

    return res.json() as Promise<{ success: boolean; message?: string }>;
  } catch {
    return { success: false, message: "Service unavailable" };
  }
}

// ── AI ─────────────────────────────────────────────────────────────

export async function askTearawhiti(question: string): Promise<{ answer: string; enabled: boolean }> {
  const data = await fetchTearawhitiData(["tearawhiti:treaty-settlements", "tearawhiti:engagements"]);
  if (!data) {
    return { answer: "Unable to load your Te Arawhiti information.", enabled: false };
  }
  const context = produceTearawhitiAiContext(data);
  return askWithContext(question, context);
}
