import Link from "next/link";
import { fetchMaritimeData } from "../actions";

export const metadata = { title: "Vessels — Maritime New Zealand — My Gov NZ" };

export default async function MaritimeVesselsPage() {
  const data = await fetchMaritimeData(["maritime:vessels"]);
  if (!data) {
    return (
      <main>
        <h1>Vessels</h1>
        <p>Unable to load your Maritime information.</p>
        <Link href={"/consent?dept=maritime"}>Grant Maritime access</Link>
      </main>
    );
  }

  const rows = data.vessels ?? [];

  return (
    <main>
      <Link href={"/dept/maritime"}>← Back to Maritime</Link>
      <h1>Vessels</h1>
      {rows.length === 0 ? (
        <p>No vessels on file.</p>
      ) : (
        <table>
          <thead>
            <tr>
                <th>vesselName</th>
                <th>flag</th>
                <th>status</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => (
              <tr key={i}>
                  <td>{row.vesselName}</td>
                  <td>{row.flag}</td>
                  <td>{row.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
      )}
    </main>
  );
}
