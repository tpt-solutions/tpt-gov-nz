"use server";

import type { EroDataBundle } from "@tpt/gov-schema";
import { askWithContext } from "../../ai/client";
import { fetchDeptData } from "../../lib/data-access";
import { getCitizenDid } from "../../lib/session";
import { produceEroAiContext } from "./ero-ai";

const ERO_SERVICE_URL = process.env.ERO_SERVICE_URL ?? "http://localhost:8140";

export async function fetchEroData(scopes: string[]): Promise<EroDataBundle | null> {
  return (await fetchDeptData("ero", scopes)) as EroDataBundle | null;
}

export async function submitEroAction(
  type: string,
  parameters: Record<string, unknown>
): Promise<{ success: boolean; message?: string }> {
  if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") {
    return { success: true, message: "Demo mode: action recorded locally (no live service)." };
  }

  const did = await getCitizenDid();
  if (!did) return { success: false, message: "Not authenticated" };

  try {
    const res = await fetch(ERO_SERVICE_URL + "/citizen/action", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ did, type, parameters, performed_by: "citizen" }),
    });

    if (!res.ok) {
      const err = (await res.json()) as { error?: string };
      return { success: false, message: err.error ?? "Action failed" };
    }

    return res.json() as Promise<{ success: boolean; message?: string }>;
  } catch {
    return { success: false, message: "Service unavailable" };
  }
}

// ── AI ─────────────────────────────────────────────────────────────

export async function askEro(question: string): Promise<{ answer: string; enabled: boolean }> {
  const data = await fetchEroData(["ero:reviews", "ero:reports"]);
  if (!data) {
    return { answer: "Unable to load your Education Review Office information.", enabled: false };
  }
  const context = produceEroAiContext(data);
  return askWithContext(question, context);
}
