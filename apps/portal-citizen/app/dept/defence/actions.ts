"use server";

import type { DefenceDataBundle } from "@tpt/gov-schema";
import { askWithContext } from "../../ai/client";
import { fetchDeptData } from "../../lib/data-access";
import { getCitizenDid } from "../../lib/session";
import { produceDefenceAiContext } from "./defence-ai";

const DEFENCE_SERVICE_URL = process.env.DEFENCE_SERVICE_URL ?? "http://localhost:8144";

export async function fetchDefenceData(scopes: string[]): Promise<DefenceDataBundle | null> {
  return (await fetchDeptData("defence", scopes)) as DefenceDataBundle | null;
}

export async function submitDefenceAction(
  type: string,
  parameters: Record<string, unknown>
): Promise<{ success: boolean; message?: string }> {
  if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") {
    return { success: true, message: "Demo mode: action recorded locally (no live service)." };
  }

  const did = await getCitizenDid();
  if (!did) return { success: false, message: "Not authenticated" };

  try {
    const res = await fetch(DEFENCE_SERVICE_URL + "/citizen/action", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ did, type, parameters, performed_by: "citizen" }),
    });

    if (!res.ok) {
      const err = (await res.json()) as { error?: string };
      return { success: false, message: err.error ?? "Action failed" };
    }

    return res.json() as Promise<{ success: boolean; message?: string }>;
  } catch {
    return { success: false, message: "Service unavailable" };
  }
}

// ── AI ─────────────────────────────────────────────────────────────

export async function askDefence(question: string): Promise<{ answer: string; enabled: boolean }> {
  const data = await fetchDefenceData(["defence:procurements", "defence:bases"]);
  if (!data) {
    return { answer: "Unable to load your Ministry of Defence information.", enabled: false };
  }
  const context = produceDefenceAiContext(data);
  return askWithContext(question, context);
}
