"use server";

import type { TecDataBundle } from "@tpt/gov-schema";
import { askWithContext } from "../../ai/client";
import { fetchDeptData } from "../../lib/data-access";
import { getCitizenDid } from "../../lib/session";
import { produceTecAiContext } from "./tec-ai";

const TEC_SERVICE_URL = process.env.TEC_SERVICE_URL ?? "http://localhost:8141";

export async function fetchTecData(scopes: string[]): Promise<TecDataBundle | null> {
  return (await fetchDeptData("tec", scopes)) as TecDataBundle | null;
}

export async function submitTecAction(
  type: string,
  parameters: Record<string, unknown>
): Promise<{ success: boolean; message?: string }> {
  if (process.env.NEXT_PUBLIC_DEMO_MODE === "true") {
    return { success: true, message: "Demo mode: action recorded locally (no live service)." };
  }

  const did = await getCitizenDid();
  if (!did) return { success: false, message: "Not authenticated" };

  try {
    const res = await fetch(TEC_SERVICE_URL + "/citizen/action", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ did, type, parameters, performed_by: "citizen" }),
    });

    if (!res.ok) {
      const err = (await res.json()) as { error?: string };
      return { success: false, message: err.error ?? "Action failed" };
    }

    return res.json() as Promise<{ success: boolean; message?: string }>;
  } catch {
    return { success: false, message: "Service unavailable" };
  }
}

// ── AI ─────────────────────────────────────────────────────────────

export async function askTec(question: string): Promise<{ answer: string; enabled: boolean }> {
  const data = await fetchTecData(["tec:funding", "tec:courses"]);
  if (!data) {
    return { answer: "Unable to load your Tertiary Education Commission information.", enabled: false };
  }
  const context = produceTecAiContext(data);
  return askWithContext(question, context);
}
